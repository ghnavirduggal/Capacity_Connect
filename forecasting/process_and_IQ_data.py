...
def process_forecast_results(
    ...
    for model_name, df_fc in forecast_results.items():
        if model_name in ("train", "test", "debug", "final_smoothed_values"):
            continue
        if df_fc is not None and not df_fc.empty:
            df_temp = df_fc.copy()
            df_temp["Model"] = model_name.capitalize()
            df_temp = df_temp.rename(columns={"ds": "Month", "yhat": "Forecast"})
            combined_forecast_df = pd.concat([combined_forecast_df, df_temp], ignore_index=True)
    ...
    if not combined_forecast_df.empty:
        combined_forecast_df["Month"] = (
            pd.to_datetime(combined_forecast_df["Month"]).dt.to_period("M").dt.to_timestamp()
        )
        ...
        wide_forecast_df = combined_forecast_df.pivot(index="Model", columns="Month_Year", values="Forecast").reset_index()

        if "final_smoothed_values" in forecast_results:
            final_smoothed_df = forecast_results["final_smoothed_values"].copy()
            ...
            pivot_smoothed_df = final_smoothed_df.pivot_table(
                index="Year", columns="Month", values="Final_Smoothed_Value"
            ).reset_index()
            months_order = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
            cols = ["Year"] + [m for m in months_order if m in pivot_smoothed_df.columns]
            pivot_smoothed_df = pivot_smoothed_df[cols]
...
def fill_final_smoothed_row(wide_df: pd.DataFrame, baseline_df: pd.DataFrame) -> pd.DataFrame:
    mask = wide_df["Model"] == "Final_smoothed_values"
    if not mask.any():
        return wide_df

    baseline_melted = baseline_df.melt(id_vars="Year", var_name="Month", value_name="Smoothed")
    baseline_melted["Month_Year"] = baseline_melted.apply(
        lambda row: f"{row['Month']}-{str(row['Year'])[-2:]}", axis=1
    )
    baseline_lookup = baseline_melted.set_index("Month_Year")["Smoothed"].to_dict()

    for col in wide_df.columns:
        if col == "Model":
            continue
        if col in baseline_lookup:
            wide_df.loc[mask, col] = baseline_lookup[col] / 100
